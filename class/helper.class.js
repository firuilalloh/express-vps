...
class commonHelper {
	...
	durationTimestamp(start, finish){
		const datestart = new Date(start)
		const datefinish = new Date(finish)

		const duration = date.subtract(datefinish, datestart).toSeconds()
    ...
    addSeconds(value, seconds){
        ...
        let result = date.addSeconds(new Date(value), seconds)

        return self.formatDate(result)
    }
}
